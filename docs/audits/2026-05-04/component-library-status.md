...
| Storybook story types | 81 component stories, 13 live component stories, 6 aggregate example stories, 0 component/layout page-mode stories |
| Playwright component capture | 100 Storybook routes captured |
| Capture artifacts | 100 screenshots, 100 WebM videos, 100 MP4 videos |
| Latest capture | `output/playwright/exo-ui-components/2026-05-05T16-18-17-214Z/viewer.html` |
| Browser suite | 80 Playwright tests passing |
| ExUnit suite | 503 tests passing |
| Visual regression | 100 committed screenshot baselines with pixel-diff checking |
...
| Visual proof | Automated screenshots and videos for 100 routes, committed visual baselines, a CI-friendly diff command, and GitHub Actions wiring | Needs review tuning once real PR diffs start producing visual changes |
...
| Usage docs | Central copy-paste usage reference exists for the current public component surface, plus button, app-shell, editable-record, bulk-action, bulk-edit, dashboard-drilldown, data-table, async-save, saved-filter, command-routing, role-operations, action/form, table/overlay/menu, component-state, token, and combobox recipes | Still needs more narrow per-component pages for the highest-traffic primitives |

## Remaining priorities

1. Keep expanding narrow per-component recipe pages for the highest-traffic primitives: input, select, combobox, table, modal, drawer, command palette, and date picker.
...
- `bun run test:browser` -> 80 tests, 0 failures.
- `bun run capture:components` -> 100 entries, 0 failed, 100 MP4 conversions in `output/playwright/exo-ui-components/2026-05-05T16-18-17-214Z`.
- `bun run capture:validate` -> 100 entries with non-empty screenshot, WebM, and MP4 files.
- `bun run visual:update` -> refreshed the expected screenshot baselines from the latest capture after adding Button Recipes.
- `bun run visual:check` -> 100 current screenshots matched the committed baseline.
- `docs/guides/component-usage.md` now links to button, app-shell, editable-record, bulk-action, bulk-edit, dashboard-drilldown, data-table, import-export, async-save, command-routing, role-operations, saved-filter, action/form, table/overlay/menu, component-state, and token guides.
