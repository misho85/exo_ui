...
| Storybook story types | 81 component stories, 8 live component stories, 6 aggregate example stories, 0 component/layout page-mode stories |
| Playwright component capture | 95 Storybook routes captured |
| Capture artifacts | 95 screenshots, 95 WebM videos, 95 MP4 videos |
| Latest capture | `output/playwright/exo-ui-components/2026-05-05T13-40-26-431Z/viewer.html` |
| Browser suite | 75 Playwright tests passing |
| ExUnit suite | 503 tests passing |
| Visual regression | 95 committed screenshot baselines with pixel-diff checking |
...
| Visual proof | Automated screenshots and videos for 95 routes, committed visual baselines, a CI-friendly diff command, and GitHub Actions wiring | Needs review tuning once real PR diffs start producing visual changes |
...
| Usage docs | Central copy-paste usage reference exists for the current public component surface, plus app-shell, editable-record, bulk-action, bulk-edit, async-save, saved-filter, command-routing, action/form, table/overlay/menu, component-state, token, and combobox recipes | Still needs more narrow per-component pages for the highest-traffic primitives |
...
2. Add more app-level workflow examples over time, especially production navigation shells, advanced data-table recipes, and import/export flows.
...
- `bun run test:browser` -> 75 tests, 0 failures.
- `bun run capture:components` -> 95 entries, 0 failed, 95 MP4 conversions in `output/playwright/exo-ui-components/2026-05-05T13-40-26-431Z`.
- `bun run capture:validate` -> 95 entries with non-empty screenshot, WebM, and MP4 files.
- `bun run visual:update` -> refreshed the expected screenshot baselines from the latest capture after adding Bulk Edit Workflow.
- `bun run visual:check` -> 95 current screenshots matched the committed baseline.
- `docs/guides/component-usage.md` now links to app-shell, editable-record, bulk-action, bulk-edit, async-save, command-routing, saved-filter, action/form, table/overlay/menu, component-state, and token guides.
