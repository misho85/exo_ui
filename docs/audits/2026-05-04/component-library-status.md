...
| Storybook story types | 81 component stories, 7 live component stories, 6 aggregate example stories, 0 component/layout page-mode stories |
| Playwright component capture | 94 Storybook routes captured |
| Capture artifacts | 94 screenshots, 94 WebM videos, 94 MP4 videos |
| Latest capture | `output/playwright/exo-ui-components/2026-05-05T13-26-16-849Z/viewer.html` |
| Browser suite | 74 Playwright tests passing |
| ExUnit suite | 503 tests passing |
| Visual regression | 94 committed screenshot baselines with pixel-diff checking |
...
| Visual proof | Automated screenshots and videos for 94 routes, committed visual baselines, a CI-friendly diff command, and GitHub Actions wiring | Needs review tuning once real PR diffs start producing visual changes |
...
| Usage docs | Central copy-paste usage reference exists for the current public component surface, plus app-shell, editable-record, bulk-action, async-save, saved-filter, command-routing, action/form, table/overlay/menu, component-state, token, and combobox recipes | Still needs more narrow per-component pages for the highest-traffic primitives |
...
2. Add more app-level workflow examples over time, especially bulk edit success paths, production navigation shells, and advanced data-table recipes.
...
- `bun run test:browser` -> 74 tests, 0 failures.
- `bun run capture:components` -> 94 entries, 0 failed, 94 MP4 conversions in `output/playwright/exo-ui-components/2026-05-05T13-26-16-849Z`.
- `bun run capture:validate` -> 94 entries with non-empty screenshot, WebM, and MP4 files.
- `bun run visual:update` -> refreshed the expected screenshot baselines from the latest capture after adding Saved Filters Workflow.
- `bun run visual:check` -> 94 current screenshots matched the committed baseline.
- `docs/guides/component-usage.md` now links to app-shell, editable-record, bulk-action, async-save, command-routing, saved-filter, action/form, table/overlay/menu, component-state, and token guides.
