...
| Latest capture | `output/playwright/exo-ui-components/2026-05-06T17-31-45-182Z/viewer.html` |
...
- `bun run capture:components` -> 114 entries, 0 failed, 114 MP4 conversions in `output/playwright/exo-ui-components/2026-05-06T17-31-45-182Z`.
- `bun run capture:validate` -> 114 entries with non-empty screenshot, WebM, and MP4 files.
- `bun run visual:update` -> refreshed the expected screenshot baselines from the latest capture after the theme-toggle icon pass.
