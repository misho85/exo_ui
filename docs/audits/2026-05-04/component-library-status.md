...
| Storybook story types | 81 component stories, 6 live component stories, 6 aggregate example stories, 0 component/layout page-mode stories |
| Playwright component capture | 93 Storybook routes captured |
| Capture artifacts | 93 screenshots, 93 WebM videos, 93 MP4 videos |
| Latest capture | `output/playwright/exo-ui-components/2026-05-05T13-12-52-433Z/viewer.html` |
| Browser suite | 73 Playwright tests passing |
| ExUnit suite | 503 tests passing |
| Visual regression | 93 committed screenshot baselines with pixel-diff checking |
...
| Visual proof | Automated screenshots and videos for 93 routes, committed visual baselines, a CI-friendly diff command, and GitHub Actions wiring | Needs review tuning once real PR diffs start producing visual changes |
...
2. Add more app-level workflow examples over time, especially saved filters, bulk edit success paths, and production navigation shells.
...
- `bun run test:browser` -> 73 tests, 0 failures.
- `bun run capture:components` -> 93 entries, 0 failed, 93 MP4 conversions in `output/playwright/exo-ui-components/2026-05-05T13-12-52-433Z`.
- `bun run capture:validate` -> 93 entries with non-empty screenshot, WebM, and MP4 files.
- `bun run visual:update` -> refreshed the expected screenshot baselines from the latest capture after adding Command Routing Workflow.
- `bun run visual:check` -> 93 current screenshots matched the committed baseline.
- `docs/guides/component-usage.md` now links to app-shell, editable-record, bulk-action, async-save, command-routing, action/form, table/overlay/menu, component-state, and token guides.
