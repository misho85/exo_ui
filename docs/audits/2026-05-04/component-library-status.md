...
| Storybook story types | 81 component stories, 12 live component stories, 6 aggregate example stories, 0 component/layout page-mode stories |
| Playwright component capture | 99 Storybook routes captured |
| Capture artifacts | 99 screenshots, 99 WebM videos, 99 MP4 videos |
| Latest capture | `output/playwright/exo-ui-components/2026-05-05T15-38-47-563Z/viewer.html` |
| Browser suite | 79 Playwright tests passing |
| ExUnit suite | 503 tests passing |
| Visual regression | 99 committed screenshot baselines with pixel-diff checking |
...
| Visual proof | Automated screenshots and videos for 99 routes, committed visual baselines, a CI-friendly diff command, and GitHub Actions wiring | Needs review tuning once real PR diffs start producing visual changes |
...
| Usage docs | Central copy-paste usage reference exists for the current public component surface, plus app-shell, editable-record, bulk-action, bulk-edit, dashboard-drilldown, data-table, async-save, saved-filter, command-routing, role-operations, action/form, table/overlay/menu, component-state, token, and combobox recipes | Still needs more narrow per-component pages for the highest-traffic primitives |
...
- `bun run test:browser` -> 79 tests, 0 failures.
- `bun run capture:components` -> 99 entries, 0 failed, 99 MP4 conversions in `output/playwright/exo-ui-components/2026-05-05T15-38-47-563Z`.
- `bun run capture:validate` -> 99 entries with non-empty screenshot, WebM, and MP4 files.
- `bun run visual:update` -> refreshed the expected screenshot baselines from the latest capture after adding Role Operations Workflow.
- `bun run visual:check` -> 99 current screenshots matched the committed baseline.
- `docs/guides/component-usage.md` now links to app-shell, editable-record, bulk-action, bulk-edit, dashboard-drilldown, data-table, import-export, async-save, command-routing, role-operations, saved-filter, action/form, table/overlay/menu, component-state, and token guides.
