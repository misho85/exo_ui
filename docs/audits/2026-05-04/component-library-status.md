...
| Storybook story types | 81 component stories, 11 live component stories, 6 aggregate example stories, 0 component/layout page-mode stories |
| Playwright component capture | 98 Storybook routes captured |
| Capture artifacts | 98 screenshots, 98 WebM videos, 98 MP4 videos |
| Latest capture | `output/playwright/exo-ui-components/2026-05-05T15-02-08-837Z/viewer.html` |
| Browser suite | 78 Playwright tests passing |
| ExUnit suite | 503 tests passing |
| Visual regression | 98 committed screenshot baselines with pixel-diff checking |
...
| Visual proof | Automated screenshots and videos for 98 routes, committed visual baselines, a CI-friendly diff command, and GitHub Actions wiring | Needs review tuning once real PR diffs start producing visual changes |
...
| Usage docs | Central copy-paste usage reference exists for the current public component surface, plus app-shell, editable-record, bulk-action, bulk-edit, dashboard-drilldown, data-table, async-save, saved-filter, command-routing, action/form, table/overlay/menu, component-state, token, and combobox recipes | Still needs more narrow per-component pages for the highest-traffic primitives |
...
2. Add more app-level workflow examples over time, especially production navigation shells and role-specific operations views.
...
- `bun run test:browser` -> 78 tests, 0 failures.
- `bun run capture:components` -> 98 entries, 0 failed, 98 MP4 conversions in `output/playwright/exo-ui-components/2026-05-05T15-02-08-837Z`.
- `bun run capture:validate` -> 98 entries with non-empty screenshot, WebM, and MP4 files.
- `bun run visual:update` -> refreshed the expected screenshot baselines from the latest capture after adding Dashboard Drilldown Workflow.
- `bun run visual:check` -> 98 current screenshots matched the committed baseline.
- `docs/guides/component-usage.md` now links to app-shell, editable-record, bulk-action, bulk-edit, dashboard-drilldown, data-table, import-export, async-save, command-routing, saved-filter, action/form, table/overlay/menu, component-state, and token guides.
