...
| Latest capture | `output/playwright/exo-ui-components/2026-05-04T15-40-02-742Z/viewer.html` |
...
- Modal, sheet, and drawer now share stronger overlay focus isolation: open overlays inert outside page siblings, trap Tab/Shift+Tab inside the dialog, keep Escape scoped to the active/topmost overlay, and restore focus to the trigger after close.
...
| Overlays/menus | Browser-tested popover, dropdown, context menu, menubar, modal/sheet/drawer focus traps, topmost Escape handling, outside inerting, and focus restore | Body scroll-lock and pointer-outside transition edge cases still need deeper Radix/shadcn parity checks |
...
2. Continue overlay parity work with body scroll locking, nested overlay content examples, and pointer-outside behavior under transition timing.
...
- `bun run capture:components` -> 83 entries, 0 failed, 83 MP4 conversions in `output/playwright/exo-ui-components/2026-05-04T15-40-02-742Z`.
