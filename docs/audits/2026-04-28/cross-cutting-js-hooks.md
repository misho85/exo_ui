...
- **Fix recipe:** Emit these attributes from the HEEx templates. Hooks should _verify_, not _create_.
...
- **Impact:** Setting the same attribute repeatedly is a no-op for HTML, but if `_findControl` selects a _different_ element on re-bind (e.g. content slot reordered), the _old_ element retains forced attributes. No cleanup of mutations made to the inner element.
